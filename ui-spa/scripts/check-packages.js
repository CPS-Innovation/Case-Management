...
//convert it into the format {name: "package-name", versions: ["x.y.z", "a.b.c"]}
const getJSONInput = () => {
  return input
    .trim()
    .split("\n")
    .map((line) => {
      ...
      return {
        name: name.trim(),
        versions: versions.split(",").map((v) => v.trim()),
      };
    });
};

const packages = getJSONInput();

// Check all packages in the list

let totalChecked = 0;
let totalInstalled = 0;
let totalNotInstalled = 0;

function findVersionInTree(tree, pkgName, version) {
  if (!tree.dependencies) return false;
  for (const dep in tree.dependencies) {
    if (dep === pkgName && tree.dependencies[dep].version === version) {
      return true;
    }
    if (findVersionInTree(tree.dependencies[dep], pkgName, version)) {
      return true;
    }
  }
  return false;
...
function checkPackageVersionsWithCount(pkg) {
  pkg.versions.forEach((version) => {
    totalChecked++;
    try {
      const result = execSync(`npm ls ${pkg.name} --all --json`, {
        encoding: "utf8",
        stdio: "pipe",
      });
      const tree = JSON.parse(result);
      if (findVersionInTree(tree, pkg.name, version)) {
        totalInstalled++;
        console.log(`\x1b[31m${pkg.name}@${version} is installed.\x1b[0m`);
      } else {
        totalNotInstalled++;
        console.log(`${pkg.name}@${version} is NOT installed.`);
      }
    } catch (e) {
      totalNotInstalled++;
      console.log(`${pkg.name}@${version} is NOT installed.`);
...
packages.forEach(checkPackageVersionsWithCount);

console.log("\nSummary:");
console.log(`Total packages checked: ${totalChecked}`);
console.log(`Installed: \x1b[31m${totalInstalled}\x1b[0m`);
console.log(`Not installed: ${totalNotInstalled}`);
