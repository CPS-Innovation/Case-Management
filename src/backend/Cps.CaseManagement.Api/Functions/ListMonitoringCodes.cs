...
public class ListMonitoringCodes(ILogger<ListMonitoringCodes> logger,
  IMdsClient mdsClient,
  IMdsArgFactory mdsArgFactory)
{
  private readonly ILogger<ListMonitoringCodes> _logger = logger;
  ...
  public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "v1/monitoring-codes")] HttpRequest req, FunctionContext functionContext)
  {
    ...
    var result = await _mdsClient.GetMonitoringCodesAsync(_mdsArgFactory.CreateBaseArg(context.CmsAuthValues, context.CorrelationId));
