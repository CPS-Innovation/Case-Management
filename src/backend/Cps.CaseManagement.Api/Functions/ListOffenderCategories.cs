...
public class ListOffenderCategories(ILogger<ListOffenderCategories> logger,
  IMdsClient mdsClient,
  IMdsArgFactory mdsArgFactory)
{
  private readonly ILogger<ListOffenderCategories> _logger = logger;
  ...
  public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "v1/offender-categories")] HttpRequest req, FunctionContext functionContext)
  {
    ...
    var result = await _mdsClient.GetOffenderCategoriesAsync(_mdsArgFactory.CreateBaseArg(context.CmsAuthValues, context.CorrelationId));
